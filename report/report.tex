...
\usepackage{listings}
\lstset{breaklines=true, frame=single, numbers=left, basicstyle=\ttfamily}
% Diagrams
\usepackage{graphicx}
% Graph
\usepackage{tikz}
...
% For front page
\usepackage[left=1.9in, right=1.9in, top=1.4in, bottom=1.0in]{geometry}
% references
\usepackage[square,sort,comma,numbers]{natbib} %References
% urls in references
\usepackage[hyphens]{url}
% to make toc clickable
\usepackage{hyperref}
\hypersetup{
    colorlinks,
    citecolor=black,
...
% Title Page
% This was taken from the template provided by the department
\thispagestyle{empty}
...
Just how essential good testing is for developing correct compilers was demonstrated by John Regehr et al.\ with their Csmith project \cite{Yang:2011:FUB:1993316.1993532}. By fuzzy-testing\footnote{Generating a large number of random but valid C programs.} C compilers it was possible to find around 325 bugs in both GCC and LLVM. Besides these two compilers, a third compiler for a limited subset of C described in \cite{Leroy:2009:FVR:1538788.1538814} was tested. This compiler, called CompCert, was proven correct by its author using formal verification methods. Developing such proofs for critical parts of the LJSP compiler would be one possible starting point for future work.
...
\item A ray tracer written in JavaScript that was used to test the generated asm.js code specifically.
\end{itemize}
The following two sections will describe both systems in detail.

\section{Testing using \texttt{run_tests.py}}
The \texttt{run_tests.py} file contains a custom testing framework that compares the results of different compilation stages with each other. It is based on the fact that no transformation should change the result of the program.
...
The initial result of asm.js code being much slower than plain Java\-Script were surprising and going against our expectations. Although we were able to much improve the asm.js code generated by our compiler, this performance gap did not go away. 
...
Finally, additional intermediate stages could be added that optimise the generated code more heavily such as tail call elimination. The intermediate representation that is already part of the compiler offers a suitable framework for adding such stages. This would depend on the addition proposed in the previous paragraph, as these new stages would have to be tested to be reliable. Especially optimisation stages are prone to contain errors and keeping these new stages as small as possible, as described in \cite{Keep:2013:NFC:2544174.2500618} would make testing and debugging them easier.
...
\section{Compiler}
The first step before the compiler can be used is to compile the compiler itself. This is done by calling \texttt{scalac -deprecation ./*.scala} from the root directory of the repository. We have included an executable version that was compiled with Scala version 2.9.1-1. Compiling the compiler will take some time (around 37 seconds on a 1.6 GHz MacBook Air).
...
\item One compilation target. The full list of possible targets can be viewed by calling the compiler without arguments and corresponds to the list of stages in the compiler. If no target is given, the output of all stages is printed to the command line.
...
\addcontentsline{toc}{chapter}{Appendix: Source Code Listing}
