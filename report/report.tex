...
\begin{figure}[ht]
\begin{alignat*}{2}
% TODO free_vars function
&\mathcal{C}\llbracket (\text{lambda}\ (p_1, \dots, p_n)\ e) \rrbracket &&\eqdef k((\text{lambda}\ (f_{cont}, p_1, \dots, p_n)\ \\
&&&\hspace{1cm}\cpstrans{e}f_{cont})) \\
%
...
\label{cloconv}
\end{figure}
@@ Together with CPS-translation, closure conversion forms the core of the frontend conversions of the LJSP compiler. Closure conversion is necessary when converting from languages that allow functions as first-class objects, such as LISP, to languages that do not have that property, such as C.
...
% overview: structure of code, different files etc.
% different stages (parser implemented using java lib, cps translation two functions b/c of differing types)
