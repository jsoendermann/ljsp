...
% TODO ~~floor conversion is slow, but not actually that slow. mention it
% TODO ray tracer implementation: features like antialiasing were added until the computations were complex enough to take some time

\maketitle

%\onehalfspacing

% TODO word count on this page
\begin{center}
...
% TODO replace "substantial margin" with hard numbers
...
% TODO mention further down that the mask doesn't seem to be required
Similarly to function table calles explained above, accessing the values in an ArrayBuffer requires addtional safeguards to make sure the result is not undefined. According to the asm.js spec, array dereference expressions must be of the form \texttt{x[(e \& mask) {>}> i]} with \texttt{mask} being equal to the number of bytes in the ArrayBuffer\footnote{Which must be a multiple of $4096$} and \texttt{i} being equal to the number of bytes the data type of the view used for the access is based on (e.g. $4$ in the case of \texttt{Int32View}). In combination, they prevent out-of-bounds accesses by discarding bits larger than the length of the array and shifting the index to the right size for the view.
...
% TODO example program (fib) with mathematical definition
...
% TODO concerning this structure: clarify that it is made up of allocated memory and that there is no structs anywhere in the code
...
% TODO mention that IR code doesn't get tested because it was a late addition to the compiler and there is only one simple optimisation so far. if development on LJSP continues, an interpreter for IR code will have to be written
...
% TODO describe structure of the code
% TODO testing process: first evaluate source file, then front then back ends
...
% TODO make sure you mention that based on the fact described in the design chapter that stages should not affect the result that the program evaluates to
...
% TODO check if no other validator is available
...
\chapter{Evaluation}
% TODO evaluate the use of scala. slow!!
...
\section{Benchmarks}
% TODO show profiler image in evaluation chapter to prove that much time is spent in raySPhere...
...
% TODO link to the blogpost that shows the engine
% TODO cite http://arewefastyet.com/#machine=11&view=breakdown&suite=asmjs-apps
...
% TODO many stages only make changes to one particular expression but need to recurse over all expressions. it would be nice if a recursive caller could be implemented for this
...
% TODO something about how especiall optimisation stages are prone to be full of bugs




% TODO testing: mention that IR isn't tested because there aren't many IR stages yet and the initial IR conversion was copied from asm.js when C was added and that that part had been tested
% TODO in evaluation or conclusion: say that asm.js is very limited and might not be useful far outside the example that mozilla gave (game engine)
% TODO mention somewhere how limited asm.js is (arithmetic operations + reading and writing from array, quote "why asm.js bothers me") and that it's surprising that it does so badly for mathematical functions
...
% TODO links to hn asm.js discussions, blogposts


\chapter{User Guide}
% TODO mention versions of programs used for all 
...
The ray tracer can be used by opening the \texttt{ray_tracer/ray_tracer.html} file in a recent version of Firefox. Clicking on one of the four buttons will render a 3D scene with the rendering method selected. Please note that the renderer will not show anything until the renderer has finished. 
...
% TODO link to Emscripten tutorial

\section{Testing}
% TODO mention programs required (racket, cc, lli) and setting paths
...
% TODO better explanation, links to racket, llvm
