...
\section{asm.js}
\begin{itemize}
\item history, conception
\item general idea
\item idea of static typing/type coercion
\item specification, parts that are used in the module
\end{itemize}
...
\section{Ray tracing}
\begin{itemize}
\item what a renderer is
\item the idea behind ray tracing
\item how to do reflection
\end{itemize}
...
This stage takes as its input the C AST generated by the "Conversion to C" stage. The reason for this is that 
% TODO mention SSA, maybe in the background chapter?
\subsection{Conversion to Numbered LLVM IR}
% TODO maybe mention in background chapter

\section{Design of the Ray Tracer}
% TODO describe idea behind ray tracer in bg chapter
% TODO talk about why using a ray tracer and why adding the features below
After giving a brief sketch of the idea behind ray tracing in the Background chapter, this section will describe the capabilities of the ray tracer included in the project. It will also explain the mathematical theory behind the calculations necessary for ray tracing.
...
\item Shading \footnote{Varying intensities of light. Visible on the floor of the room in the example screen shot in Figure \ref{raytracerscreenshot}.} based on the angle from the surface normal to the light source
